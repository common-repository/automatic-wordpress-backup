...
if (!class_exists('wdc_settings')) {
	class wdc_settings {
		...
		function admin_menu() {
			add_menu_page('Volcanic', 'Volcanic', 'level_10', apply_filters('wdc-settings-url', 'volcanic-credits'), apply_filters('wdc-settings-page', array(&$this, 'settings')), wdc_settings::get_url() . 'wdc-icon.png'); 
			do_action('wdc-menu-pages');
			add_submenu_page(apply_filters('wdc-settings-url', 'volcanic-credits'), 'Credits', 'Credits', 'level_10', 'volcanic-credits', array(&$this, 'settings')); 
		}
